...
export default function MessageSender() {
  ...
  const handleSend = () => {
    ...
    // Run Dijkstra
    const route = findRoute(sourcePlanet, targetPlanet, universeConfig, offlinePlanets);
  ...
  const availablePlanets = universeConfig.nodes.filter(n => !offlinePlanets.has(n.id));
    ...
    <div className="bg-neutral-900/50 backdrop-blur-md p-6 rounded-xl border border-white/5 flex flex-col gap-4">
      ...
      <div className="flex gap-4">
        <div className="flex-1 flex flex-col gap-1">
          <label className="text-[10px] text-cyan-500/70 uppercase font-bold tracking-wider">Source</label>
          <select 
            value={sourcePlanet || ''}
        ...
        <div className="flex-1 flex flex-col gap-1">
          <label className="text-[10px] text-cyan-500/70 uppercase font-bold tracking-wider">Destination</label>
          <select 
            value={targetPlanet || ''}
      ...
      <div className="flex flex-col gap-1">
        <label className="text-[10px] text-cyan-500/70 uppercase font-bold tracking-wider">Payload</label>
        <textarea 
          value={messagePayload}
          onChange={(e) => setMessagePayload(e.target.value)}
          placeholder="ENTER MESSAGE..."
          rows={3}
          className="bg-neutral-950 border border-white/10 rounded p-2 text-sm text-cyan-100 font-mono focus:outline-none focus:border-cyan-500/50 resize-none uppercase"
        />
      </div>
