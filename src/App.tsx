...
function App() {
  ...
  // Mobile Tabs State
  const [activeTab, setActiveTab] = useState<"controls" | "metrics">(
    "controls",
  );
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center w-screen h-screen bg-neutral-950 text-cyan-500 font-mono p-8 text-center">
        <h1 className="text-xl font-bold mb-4 uppercase tracking-widest text-red-500">
          System Initialization Halted
        </h1>
        <p className="max-w-2xl text-sm opacity-80">{error}</p>
        <p className="mt-4 text-xs opacity-50">
          Please ensure valid universe-config.json is placed in the public
          directory.
        </p>
      </div>
    );
  ...
  return (
    <div className="w-screen h-screen bg-neutral-950 overflow-hidden text-cyan-100 flex flex-col md:grid md:grid-cols-[1fr_2fr_1fr] md:gap-4 md:p-4">
      ...
      <div
        className={`md:flex flex-col gap-4 overflow-y-auto custom-scrollbar pr-1 ${activeTab === "metrics" ? "flex flex-1 p-4 md:p-0 bg-neutral-950 md:bg-transparent" : "hidden"}`}
      >
        <MetricsPanel />
        <CodexPanel />
      </div>
